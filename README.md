...
- 최초 사용 시 한 번만 표시되는 단계식 Android 13+ 오디오/알림 권한 안내. 이 흐름에서 거절한 권한이 있으면 완료 상태를 저장하지 않아 다음 실행 때만 다시 안내. “다른 앱 위에 표시”는 앱 진입 시 요구하지 않고 플로팅 가사를 켤 때만 설정 화면으로 이동
...
- 좋아요/많이 들은 곡 고정 앨범, 사용자 앨범 목록형/커버형 전환(첫 곡 커버 썸네일), 1-depth 폴더 생성·추가·root 이동·이름 변경·해체, 폴더 안 앨범과 전용 곡 목록 열기. 앨범에 곡을 추가하면 끝에 넣고 전용 목록에서 길게 눌러 순서를 편집한다
