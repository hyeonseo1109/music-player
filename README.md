...
- 삼성뮤직 연동은 설정의 “내 앨범 데이터”에 모았다. Android 공용 MediaStore에 공개된 재생목록은 동기화로 가져오고, 삼성뮤직의 내보낸 재생목록 파일(`.m3u`, `.m3u8`, `.pls`)도 파일 선택기로 가져온다. HendoMusic 내 앨범은 표준 `.m3u`로 내보내 삼성뮤직에서 가져올 수 있다. 원본 음악 파일은 복사하지 않으며, 파일 가져오기 때 찾지 못한 곡 수를 결과로 표시
