...
- 최초 사용 시 한 번만 표시되는 단계식 Android 13+ 오디오/알림 권한 안내. 저장된 권한 설정을 DataStore에서 읽기 전에는 온보딩을 렌더링하지 않아 승인된 기기에서 화면이 깜박이지 않는다. 이 흐름에서 거절한 권한이 있으면 완료 상태를 저장하지 않아 다음 실행 때만 다시 안내. “다른 앱 위에 표시”는 앱 진입 시 요구하지 않고 플로팅 가사를 켤 때만 설정 화면으로 이동
...
- 전체 곡, 실시간/한글 초성 검색, 정렬 저장, MusicRow/곡 메뉴. 길게 눌러 한 곡 또는 여러 곡을 선택하고 선택한 곡만 큐로 만드는 선택 재생. 선택 중에는 Mini Player 대신 하단 작업 바에서 재생·현재재생목록 추가·내앨범 추가·일괄 삭제를 제공하며, 검색 포커스/탭 이동 시 선택을 해제
...
- Now Playing 가사 줄 탭 seek, 앨범커버/2줄 미리보기와 가사 헤더의 양방향 모드 전환, 재생 컨트롤 위의 좋아요·내 앨범 추가·큐 바로가기, 선택 곡의 재생·큐 추가·내 앨범 일괄 추가
- 가사 검색 진입 즉시 제목/아티스트로 검색하고, 싱크 없음 표기·통일된 전체 가사 확인·적용 동작을 제공. 많이 들은 곡은 설정에서 집계와 앨범 표시를 함께 끌 수 있음
- PlaybackService가 관리하는 직접 A-B 구간 반복과 현재 위치 기준 이전 3초/5초/10초 즉시 반복
- drag handle로 순서를 바로 바꾸는 실제 Media3 playlist, Play Next, 현재 목록과 직전 목록을 서로 교체하는 “이전 불러오기”, Room 큐·현재 곡·위치·repeat·shuffle 저장/복원
- 삭제된 곡을 제외한 안전한 큐 복원, 재실행 시 자동 재생하지 않는 paused 복원
- 좋아요/많이 들은 곡 고정 앨범은 팝업이 아닌 전용 곡 목록으로 열리고, 사용자 앨범은 목록형/커버형 전환(첫 곡 커버 썸네일)을 제공한다. 폴더는 최대 네 앨범의 첫 곡 커버를 여백 있는 2×2 콜라주 썸네일로 표시하며, 1-depth 폴더 생성·추가·root 이동·이름 변경·해체와 폴더 안 앨범/전용 곡 목록 열기를 지원한다. 앨범에 곡을 추가하면 끝에 넣고 전용 목록에서 길게 눌러 순서를 편집한다
- 삼성뮤직 연동은 설정의 “내 앨범 데이터”에 모았다. Android 공용 MediaStore에 공개된 재생목록은 동기화로 가져오고, 삼성뮤직 내보내기 파일(`.smpl`)과 표준 재생목록 파일(`.m3u`, `.m3u8`, `.pls`)도 파일 선택기로 가져온다. `.smpl`의 Samsung Music `order`와 절대 저장소 경로를 읽어 현재 보관함의 상대 경로/파일명에 매칭한다. HendoMusic 내 앨범은 표준 `.m3u`로 내보내 삼성뮤직에서 가져올 수 있다. 원본 음악 파일은 복사하지 않으며, 파일 가져오기 때 찾지 못한 곡 수를 결과로 표시
...
추가 Galaxy E2E에서 `You & I / One Direction`(로컬 240.559초)은 재생 전 Room에 lyrics row가 없음을 읽기 전용으로 확인했습니다. 수동 가사 검색·선택·편집은 사용하지 않았고, 재생 trigger 뒤 LRCLIB 요청은 HTTP 200, 후보는 20개였으며 title/artist가 일치하고 길이 차가 2.559초인 238.0초 synced 후보가 `AUTO_LRCLIB`으로 49줄 저장됐습니다. Now Playing에 현재 줄이 표시됐습니다. Now Playing과 별도 가사 화면은 Room Flow를 관찰하므로 background 저장 결과도 화면을 다시 열지 않고 갱신합니다.
